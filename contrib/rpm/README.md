...
The RPM spec file provided here is for Pericloin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://pericloin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Pericloin git repository](https://github.com/pericloin/pericloin) but are not part of
...
Currently only `Source10` is used, for the example `pericloin.conf` file.
...
in the pericloin git repository. At present only an SVG version of the Pericloin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the pericloin
...
needed when 0.12.1 is released, a proper fix is already in the Pericloin git
master branch.
...
idea to build Pericloin Core against a static version of that library compiled
...
The `pericloin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `pericloin-server` package. At this point, most RPM based
...
The `pericloin-server` package should have SELinux support. How to properly do
...
Pericloin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/pericloin/). He has been
packaging Pericloin for Fedora at least since 2012.
...
4. I renamed the `pericloin` package that contains the Qt GUI to `pericloin-core` as
that appears to be how the general population refers to it, in contrast to
`pericloin-xt` or `pericloin-classic`. I wanted to make sure the general population
