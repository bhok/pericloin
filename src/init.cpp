 ...
 * between pericloind, and pericloin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("pericloin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), PERICLOIN_CONF_FILENAME));
    if (mode == HMM_PERICLOIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), PERICLOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/pericloin/pericloin>";
    const std::string URL_WEBSITE = "<https://pericloincore.org>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("pericloin-loadblk");
...
/** Sanity checks
 *  Ensure that Pericloin is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Pericloin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", PERICLOIN_CONF_FILENAME)).string());
